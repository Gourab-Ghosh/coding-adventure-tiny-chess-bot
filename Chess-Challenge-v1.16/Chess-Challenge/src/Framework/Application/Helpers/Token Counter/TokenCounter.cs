...
namespace ChessChallenge.Application
{
    public static class TokenCounter
    {
        ...
        public static int CountTokens(string code)
        {
            ...
            return CountTokens(root, code);
        }

        static int CountTokens(SyntaxNodeOrToken syntaxNode, string code)
        {
            ...
            {
                numTokensInChildren += CountTokens(child, code);
            }
            var lines = code.Substring(syntaxNode.FullSpan.End).Split(Environment.NewLine);
            bool endsWithIgnore = lines.FirstOrDefault()?
                .TrimEnd()
                .EndsWith("// ignore line", StringComparison.OrdinalIgnoreCase) == true;
            if (syntaxNode.IsToken && !tokensToIgnore.Contains(kind) && !endsWithIgnore)
            {
