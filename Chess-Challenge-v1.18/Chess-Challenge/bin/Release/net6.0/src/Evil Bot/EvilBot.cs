...
namespace ChessChallenge.Example
{
    ...
    static ulong[] positionValuesCompressedIndices = {
        ...
        0x7777ffff99bd889c, 0x777b865789937777, // Pawn opening position indices
        0x7777ffff99bd889c, 0x777b865789937777, // Pawn endgame position indices
        0x1221377279a28ab, 0x27ab289a13780122, // Knight opening position indices
        0x1221377279a28ab, 0x27ab289a13780122, // Knight endgame position indices
        0x3555577757895889, 0x5799599958773555, // Bishop opening position indices
        0x3555577757895889, 0x5799599958773555, // Bishop endgame position indices
        0x7777899967776777, 0x6777677767777778, // Rook opening position indices
        0x7777899967776777, 0x6777677767777778, // Rook endgame position indices
        0x3456477757886788, 0x6788578847773456, // Queen opening position indices
        0x3456477757886788, 0x6788578847773456, // Queen endgame position indices
        ...
        // 0XDDDDAAAA88887777, 0X6666555544446666, // Pawn opening position indices
        // 0XDDDDCCCCBBBBAAAA, 0X8888777755556666, // Pawn endgame position indices
        // 0X12213662689279A, 0X269A278913670122, // Knight opening position indices
        // 0X12213662689279A, 0X269A278913660122, // Knight endgame position indices
        // 0X3444466646784778, 0X4688488847663444, // Bishop opening position indices
        // 0X111122212331236, 0X1236123312220111, // Bishop endgame position indices
        // 0X6666788856665666, 0X5666566656666667, // Rook opening position indices
        // 0X2222222222222222, 0X2222222222222222, // Rook endgame position indices
        // 0X3445466646775677, 0X5677467746663445, // Queen opening position indices
        // 0X112122312342346, 0X2346123412230112, // Queen endgame position indices
        // 0X2110211021102110, 0X32214333AA66AB86, // King opening position indices
        // 0X123234624AB24BC, 0X24BC24AB22660222, // King endgame position indices
    };
    static Board board_;
    static Timer timer_;
    ...
    static int EvaluateBoard()
    {
        ...
        while (allPiecesBB != 0)
        {
            ...
            positionScore += sign * (pieceScoreAbs * GetPositionScore(compressedDataOffset, dataIndex) + (8000 - pieceScoreAbs) * GetPositionScore(compressedDataOffset + 2, dataIndex)) / 8000;
        }
    ...
    static int AlphaBetaAndQuiescence(int depth, int alpha = -30000, int beta = 30000) // Merged Quiescence and AlphaBeta into single function (depth == 0 means Quiescence search)
    {
        ...
        if (depth > 2 && Abs(beta) <= 24000)
            ...
            {
                ply++;
                int nullMoveReductionScore = -AlphaBetaAndQuiescence(depth - 3, -beta, 1-beta);
                board_.UndoSkipTurn();
                ply--;
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        { // #DEBUG
            ...
            Console.WriteLine($"info depth {depth - 1} nodes {nodes} nps {(1000 * nodes) / Max(1, timer_.MillisecondsElapsedThisTurn)} score {score}"); // #DEBUG
        } // #DEBUG
